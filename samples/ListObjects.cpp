...
int main(int argc, char* argv[]) {
    ...
    auto outcome = client.listObjects(models::ListObjectsRequest().setBucket(args.bucket).setMaxKeys(100));
    if (!outcome.isSuccess()) {
        auto& e = outcome.getError();
        std::cerr << "ListObjects fail, code: " << e.getCode() << ", message: " << e.getMessage()
    ...
    for (const auto& obj : result.getContents()) {
        std::cout << "Object: " << obj.key << ", Size: " << obj.size << ", LastModified: " << obj.lastModified
                  << ", ETag: " << obj.eTag << ", StorageClass: " << obj.storageClass << std::endl;
    }
    return 0;
}
