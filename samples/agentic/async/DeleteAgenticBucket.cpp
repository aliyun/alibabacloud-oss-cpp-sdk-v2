// Demonstrates: Async delete an agentic bucket.
#include <future>
...
int main(int argc, char* argv[]) {
    ...
    std::promise<oss::agentic::DeleteAgenticBucketOutcome> promise;
    client.deleteAgenticBucketAsync(
        oss::agentic::models::DeleteAgenticBucketRequest().setBucket(args.bucket),
        [&promise](oss::agentic::DeleteAgenticBucketOutcome outcome) { promise.set_value(std::move(outcome)); });

    auto outcome = promise.get_future().get();
    if (!outcome.has_value()) {
        auto& e = outcome.error();
        std::cerr << "DeleteAgenticBucket fail"
                  << ", code: " << e.getCode()
