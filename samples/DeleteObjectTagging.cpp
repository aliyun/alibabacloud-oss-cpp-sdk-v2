...
int main(int argc, char* argv[]) {
    ...
    auto outcome =
            client.deleteObjectTagging(models::DeleteObjectTaggingRequest().setBucket(args.bucket).setKey(args.key));
    if (!outcome.has_value()) {
        auto& e = outcome.error();
        std::cerr << "DeleteObjectTagging fail, code: " << e.getCode() << ", message: " << e.getMessage()
