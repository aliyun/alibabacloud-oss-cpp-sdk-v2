...
int main(int argc, char* argv[]) {
    ...
    auto outcome = client.getBucketLocation(models::GetBucketLocationRequest().setBucket(args.bucket));
    if (!outcome.isSuccess()) {
        auto& e = outcome.getError();
        std::cerr << "GetBucketLocation fail, code: " << e.getCode() << ", message: " << e.getMessage()
