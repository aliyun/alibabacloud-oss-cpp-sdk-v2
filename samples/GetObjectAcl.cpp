...
int main(int argc, char* argv[]) {
    ...
    auto outcome = client.getObjectAcl(models::GetObjectAclRequest().setBucket(args.bucket).setKey(args.key));
    if (!outcome.isSuccess()) {
        auto& e = outcome.getError();
        std::cerr << "GetObjectAcl fail, code: " << e.getCode() << ", message: " << e.getMessage()
