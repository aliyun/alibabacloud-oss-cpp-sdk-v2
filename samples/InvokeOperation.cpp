...
int main(int argc, char* argv[]) {
    ...
    if (std::holds_alternative<OperationError>(result)) {
        auto& e = std::get<OperationError>(result);
        std::cerr << "InvokeOperation fail, code: " << e.getCode() << ", message: " << e.getMessage()
                  << ", requestId: " << e.getRequestId() << std::endl;
        return 1;
    }

    auto& output = std::get<OperationOutput>(result);
    std::cout << "status code: " << output.statusCode << ", requestId: " << output.headers.at("x-oss-request-id")
              << std::endl;
    return 0;
}
