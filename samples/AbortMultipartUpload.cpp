...
int main(int argc, char* argv[]) {
    ...
    if (args.region.empty() || args.bucket.empty() || args.key.empty() || uploadId.empty())
        printUsageAndExit(argv[0], " --bucket <bucket> --key <key> --upload-id <id>");

    auto client = createClient(args);

    auto outcome = client.abortMultipartUpload(
            models::AbortMultipartUploadRequest().setBucket(args.bucket).setKey(args.key).setUploadId(uploadId));
    if (!outcome.has_value()) {
        auto& e = outcome.error();
        std::cerr << "AbortMultipartUpload fail, code: " << e.getCode() << ", message: " << e.getMessage()
