...
int main(int argc, char* argv[]) {
    ...
    auto outcome = client.getObjectMeta(models::GetObjectMetaRequest().setBucket(args.bucket).setKey(args.key));
    if (!outcome.isSuccess()) {
        auto& e = outcome.getError();
        std::cerr << "GetObjectMeta fail, code: " << e.getCode() << ", message: " << e.getMessage()
