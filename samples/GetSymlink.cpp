...
int main(int argc, char* argv[]) {
    ...
    if (args.region.empty() || args.bucket.empty() || args.key.empty())
        printUsageAndExit(argv[0], " --bucket <bucket> --key <symlink-key>");

    auto client = createClient(args);

    auto outcome = client.getSymlink(models::GetSymlinkRequest().setBucket(args.bucket).setKey(args.key));
    if (!outcome.has_value()) {
        auto& e = outcome.error();
        std::cerr << "GetSymlink fail, code: " << e.getCode() << ", message: " << e.getMessage()
                  << ", requestId: " << e.getRequestId() << std::endl;
        return 1;
    }
    auto& result = outcome.value();
    std::cout << "status code: " << result.getStatusCode() << ", requestId: " << result.getRequestId()
              << ", symlinkTarget: " << result.getSymlinkTarget() << ", versionId: " << result.getVersionId()
              << std::endl;
    return 0;
}
