...
int main(int argc, char* argv[]) {
    ...
    auto outcome = client.getObject(models::GetObjectRequest().setBucket(args.bucket).setKey(args.key));
    if (!outcome.has_value()) {
        auto& e = outcome.error();
        std::cerr << "GetObject fail, code: " << e.getCode() << ", message: " << e.getMessage()
    ...
    std::cout << "status code: " << result.getStatusCode() << ", requestId: " << result.getRequestId()
              ...
              << ", storageClass: " << result.getStorageClass() << ", hashCrc64ecma: " << result.getHashCrc64ecma()
              << std::endl;
