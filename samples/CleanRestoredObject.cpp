...
int main(int argc, char* argv[]) {
    ...
    auto outcome =
            client.cleanRestoredObject(models::CleanRestoredObjectRequest().setBucket(args.bucket).setKey(args.key));
    if (!outcome.isSuccess()) {
        auto& e = outcome.getError();
        std::cerr << "CleanRestoredObject fail, code: " << e.getCode() << ", message: " << e.getMessage()
