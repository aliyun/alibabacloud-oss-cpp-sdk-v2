...
struct AsyncRequestContext {
    ...
    enum class State {
        SendRequest, WriteBody, ReceiveResponse,
        QueryData, ReadData, CloseHandle
    };
    ...
    void finish() {
        ...
        struct InflightGuard {
            ...
            ~InflightGuard() { o->onRequestFinished(); }
        } guard{owner};
    ...
    bool start(HINTERNET hSession, const ConnectionOptions& connOpts) {
        ...
        if (!WinHttpSendRequest(handles.hRequest.get(),
...
void WinHttpAsyncClient::sendAsync(std::unique_ptr<RequestMessage> request,
