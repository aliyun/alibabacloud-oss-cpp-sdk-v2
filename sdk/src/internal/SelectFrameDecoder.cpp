...
SelectFrameDecodingWriter::SelectFrameDecodingWriter(std::shared_ptr<ByteWriter> inner)
...
int SelectFrameDecodingWriter::iostate() const {
    if (error_) return std::ios_base::failbit;
    if (inner_) return inner_->state();
    return 0;
}
...
void SelectFrameDecodingWriter::processBuffer() {
    ...
    while (remain > 0 && !error_) {
        if (!parseFrame(ptr, remain)) break;
    }
...
bool SelectFrameDecodingWriter::parseFrame(const std::uint8_t*& ptr, std::size_t& remain) {
    ...
    if (headerLen_ < FRAME_HEADER_LEN) {
        ...
        if (headerLen_ < FRAME_HEADER_LEN) return false;
    }
    ...
    if (payloadRemains_ > 0) {
        ...
        if (payloadRemains_ > 0) return false;
    }
...
bool SelectMetaFrameParser::parseFrame(const std::uint8_t*& ptr, std::size_t& remain) {
    ...
    if (payloadRemains_ > 0) {
        ...
        if (frameType_ == FRAME_TYPE_CSV_META_END ||
            frameType_ == FRAME_TYPE_JSON_META_END ||
    ...
    //   CSV meta end (0x800006): totalScanned(8B) + status(4B) + splitsCount(4B) + rowsCount(8B) + columnsCount(4B) + errorMessage
    //   JSON meta end (0x800007): totalScanned(8B) + status(4B) + splitsCount(4B) + rowsCount(8B) + errorMessage
    //   End frame (0x800005): same as JSON meta end
    ...
    if (frameType_ == FRAME_TYPE_CSV_META_END && endFrameData_.size() >= 28) {
        ...
        if (endFrameData_.size() > 28) {
            errorMessage_.assign(
                reinterpret_cast<const char*>(endFrameData_.data() + 28),
                endFrameData_.size() - 28);
        }
    } else if ((frameType_ == FRAME_TYPE_JSON_META_END || frameType_ == FRAME_TYPE_END)
               && endFrameData_.size() >= 24) {
        totalScanned_ = readBE64Signed(endFrameData_.data());
        status_ = readBE32Signed(endFrameData_.data() + 8);
        splitsCount_ = readBE32Signed(endFrameData_.data() + 12);
        rowsCount_ = readBE64Signed(endFrameData_.data() + 16);
        if (endFrameData_.size() > 24) {
            errorMessage_.assign(
                reinterpret_cast<const char*>(endFrameData_.data() + 24),
                endFrameData_.size() - 24);
        }
    }
